...
WITH root_data as (
    select
        ...
        COALESCE(
            ...
            (select mv.value from metadata_view as mv where mv.id = m.id and mv.property = 'https://vocabs.acdh.oeaw.ac.at/schema#hasTitle' and mv.lang = 'und' limit 1),
        ...
        COALESCE(
            ...
            (select mv.value from metadata_view as mv where mv.id = m.id and mv.property = 'https://vocabs.acdh.oeaw.ac.at/schema#hasDescription' and mv.lang = 'und' limit 1),
    ...
    WITH dmetaRel as (
        ...
        FROM (
            ...
            COALESCE(
                ...
                (select m.value from metadata as m where m.lang = 'und' and m.id = CAST(mv.value as bigint) and m.property='https://vocabs.acdh.oeaw.ac.at/schema#hasTitle' limit 1),
            ...
            left join identifiers as i on i.id = CAST(mv.value as INT) and i.ids LIKE CAST('%.acdh.oeaw.ac.at/api/%' as varchar)
            left join identifiers as i2 on i2.id = CAST(mv.value as INT) and i2.ids LIKE CAST('%vocabs.acdh.oeaw.ac.at/%' as varchar)
...
INSERT INTO collectionData(
    WITH  c2d AS (
        select
            ...
            COALESCE(
                (select mv2.value from metadata_view as mv2 where cd.mainid  = mv2.id and mv2.property = 'https://vocabs.acdh.oeaw.ac.at/schema#hasTitle' and mv2.lang = _lang limit 1),
                (select mv2.value from metadata_view as mv2 where cd.mainid  = mv2.id and mv2.property = 'https://vocabs.acdh.oeaw.ac.at/schema#hasTitle' and mv2.lang = _lang2 limit 1),
                (select mv2.value from metadata_view as mv2 where cd.mainid  = mv2.id and mv2.property = 'https://vocabs.acdh.oeaw.ac.at/schema#hasTitle' and mv2.lang = 'und' limit 1),
                (select mv2.value from metadata_view as mv2 where cd.mainid  = mv2.id and mv2.property = 'https://vocabs.acdh.oeaw.ac.at/schema#hasTitle' limit 1)
            ) as title,
...
AS $func$
    WITH t1 AS (
        ...
        FROM (
            ...
            FROM (
                SELECT
                    r1.id,
                    (array_agg(m1.value ORDER BY CASE m1.lang WHEN _lang THEN 0 WHEN 'en' THEN 1 WHEN 'de' THEN 2 WHEN 'und' THEN 3 ELSE 4 END))[1] AS ordervalue
                FROM
                    identifiers i
    ...
    FROM (
        SELECT
            ...
            (array_agg(mr2.value ORDER BY CASE mr2.lang WHEN _lang THEN 0 WHEN 'en' THEN 1 WHEN 'de' THEN 2 WHEN 'und' THEN 3 ELSE 4 END))[1] AS accesres
        FROM
            (
                SELECT
                ...
                FROM
                    (
                        SELECT
                            t2.id, orderid, title,
                            (array_agg(m3.value ORDER BY CASE m3.lang WHEN _lang THEN 0 WHEN 'en' THEN 1 WHEN 'de' THEN 2 WHEN 'und' THEN 3 ELSE 4 END))[1] AS description
                        FROM
                            (
                                SELECT
                                    t1.id, orderid,
                                    (array_agg(m2.value ORDER BY CASE m2.lang WHEN _lang THEN 0 WHEN 'en' THEN 1 WHEN 'de' THEN 2 WHEN 'und' THEN 3 ELSE 4 END))[1] AS title
                                FROM
                                    t1
...
WITH query_data as (
    ...
    COALESCE(
        (select mv2.value from metadata_view as mv2 where mv.id = mv2.id and mv2.property = 'https://vocabs.acdh.oeaw.ac.at/schema#hasTitle' and mv2.lang = _lang limit 1),
        (select mv2.value from metadata_view as mv2 where mv.id = mv2.id and mv2.property = 'https://vocabs.acdh.oeaw.ac.at/schema#hasTitle' and mv2.lang = _lang2 limit 1),
        (select mv2.value from metadata_view as mv2 where mv.id = mv2.id and mv2.property = 'https://vocabs.acdh.oeaw.ac.at/schema#hasTitle' and mv2.lang = 'und' limit 1),
        (select mv2.value from metadata_view as mv2 where mv.id = mv2.id and mv2.property = 'https://vocabs.acdh.oeaw.ac.at/schema#hasTitle' limit 1)
    ) as title,
    mv.property,
    ...
    INSERT INTO collectionData(
        WITH  c2d AS (
            select
                bcd.id,
                COALESCE(
                    (select mv2.value from metadata_view as mv2 where bcd.id = mv2.id and mv2.property = 'https://vocabs.acdh.oeaw.ac.at/schema#hasTitle' and mv2.lang = _lang limit 1),
                    (select mv2.value from metadata_view as mv2 where bcd.id = mv2.id and mv2.property = 'https://vocabs.acdh.oeaw.ac.at/schema#hasTitle' and mv2.lang = _lang2 limit 1),
                    (select mv2.value from metadata_view as mv2 where bcd.id = mv2.id and mv2.property = 'https://vocabs.acdh.oeaw.ac.at/schema#hasTitle' and mv2.lang = 'und' limit 1),
                    (select mv2.value from metadata_view as mv2 where bcd.id = mv2.id and mv2.property = 'https://vocabs.acdh.oeaw.ac.at/schema#hasTitle' limit 1)
                ) as title,
...
RETURN QUERY
    ...
    parent_subordinates AS (
        ...
        COALESCE(
            ...
            (select mv2.value from metadata_view as mv2 where mv2.id = CAST(c.value as bigint) and mv2.property = 'https://vocabs.acdh.oeaw.ac.at/schema#hasTitle' and mv2.lang = 'und' limit 1),
    ...
    Select
        ...
        COALESCE(
            ...
            (select mv2.value from metadata_view as mv2 where mv2.id = p.id and mv2.property = 'https://vocabs.acdh.oeaw.ac.at/schema#hasTitle' and mv2.lang = 'und' limit 1),
    ...
    select
        ...
        COALESCE(
            ...
            (select mv2.value from metadata_view as mv2 where mv2.id = CAST(_identifier as bigint) and mv2.property = 'https://vocabs.acdh.oeaw.ac.at/schema#hasTitle' and mv2.lang = 'und' limit 1),
...
DROP FUNCTION IF EXISTS gui.get_child_table_func(text, text, text[]  );
CREATE FUNCTION gui.get_child_table_func(_identifier text, _lang text DEFAULT 'en', _rdftype text[] DEFAULT '{}')
  RETURNS table (id bigint, property text, title text, type text, accessres text, sumcount bigint)
AS $func$
DECLARE _lang2 text := 'de';
BEGIN
...
WITH properties AS (
    ...
    where
        ...
        (select mv2.value from metadata_view as mv2 where mv2.id = iv.id  and mv2.property = 'http://www.w3.org/1999/02/22-rdf-syntax-ns#type') as type,
        COALESCE(
            (select mv2.value from metadata_view as mv left join metadata_view as mv2 on mv2.id = CAST(mv.value AS INT) where  mv.id = iv.id and mv.property = 'https://vocabs.acdh.oeaw.ac.at/schema#hasAccessRestriction' and mv2.property ='https://vocabs.acdh.oeaw.ac.at/schema#hasTitle' and mv2.lang = _lang LIMIT 1 ),
            (select mv2.value from metadata_view as mv left join metadata_view as mv2 on mv2.id = CAST(mv.value AS INT) where  mv.id = iv.id and mv.property = 'https://vocabs.acdh.oeaw.ac.at/schema#hasAccessRestriction' and mv2.property ='https://vocabs.acdh.oeaw.ac.at/schema#hasTitle' and mv2.lang = _lang2 LIMIT 1 ),
            (select mv2.value from metadata_view as mv left join metadata_view as mv2 on mv2.id = CAST(mv.value AS INT) where  mv.id = iv.id and mv.property = 'https://vocabs.acdh.oeaw.ac.at/schema#hasAccessRestriction' and mv2.property ='https://vocabs.acdh.oeaw.ac.at/schema#hasTitle' and mv2.lang = 'und' LIMIT 1 ),
            (select mv2.value from metadata_view as mv left join metadata_view as mv2 on mv2.id = CAST(mv.value AS INT) where  mv.id = iv.id and mv.property = 'https://vocabs.acdh.oeaw.ac.at/schema#hasAccessRestriction' and mv2.property ='https://vocabs.acdh.oeaw.ac.at/schema#hasTitle'  LIMIT 1 )
        ) as accessres,
            (select c.sumcount from counted as c limit 1) as sumcount
    from properties as iv
    left join resources as rs on rs.id = iv.id
    where rs.state = 'active'
	group by iv.id, iv.property;
